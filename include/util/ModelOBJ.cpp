...
void ModelOBJ::scale(float dx, float dy, float dz)
{
	float *pPosition = 0;
	for (int i = 0; i < static_cast<int>(m_vertexBuffer.size()); ++i)
	{
...
void ModelOBJ::translate(float dx, float dy, float dz)
{
	float *pPosition = 0;
	for (int i = 0; i < static_cast<int>(m_vertexBuffer.size()); ++i)
	{
...
void ModelOBJ::rotateY(float angle)
{
	float *pPosition = 0;
	...
	for (int i = 0; i < static_cast<int>(m_vertexBuffer.size()); ++i)
	{
		...
		float x = pPosition[0];
		float z = pPosition[2];
		pPosition[0] = x * _cos - z * _sin;
		pPosition[2] = x * _sin + z * _cos;
	}
}
...
void ModelOBJ::buildMeshes()
{
	...
	for (unsigned int i = 0; i < static_cast<unsigned int>(m_attributeBuffer.size()); ++i)
	{
		if (m_attributeBuffer[i] != materialId)
		{
			materialId = m_attributeBuffer[i];
...
{
	const int *pTriangle = nullptr;
	Vertex *pVertex0 = nullptr;
	Vertex *pVertex1 = nullptr;
	Vertex *pVertex2 = nullptr;
...
void ModelOBJ::generateTangents()
{
	...
	float bDotB = 0.0f;
	float length = 0.0f;
	unsigned int totalVertices = getNumberOfVertices();
	unsigned int totalTriangles = getNumberOfTriangles();
